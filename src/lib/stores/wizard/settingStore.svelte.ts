...
export class SettingStore {
  ...
  // Derived
  expandedSettingDisplay = $derived(
  ...
  async translateSetting() {
    ...
    if (
      ...
      try {
        ...
        console.error(
          "Setting translation failed (non-fatal):",
          translationError
        );
  ...
  private async translateExpandedSetting(
    ...
    if (setting.potentialConflicts?.length)
      fields.conflicts = setting.potentialConflicts.join("; ");
