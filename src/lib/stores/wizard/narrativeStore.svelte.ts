...
export class NarrativeStore {
  ...
  // Derived
  importedEntries = $derived(
    this.importedLorebooks.flatMap((lb) => lb.entries)
  );

  generatedOpeningDisplay = $derived(
    this.generatedOpeningTranslated ?? this.generatedOpening
  );
        ...
        this.generatedOpeningTranslated = {
          ...
          initialLocation: this.generatedOpening.initialLocation
            ? {
                name:
                  translated.locName ||
                  this.generatedOpening.initialLocation.name,
                description:
                  translated.locDesc ||
                  this.generatedOpening.initialLocation.description,
              }
            : this.generatedOpening.initialLocation,
        };
      } catch (translationError) {
        console.error(
          "Opening translation failed (non-fatal):",
          translationError
        );
