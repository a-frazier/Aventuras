...
{#if story.storyMode === "creative-writing"}
  ...
  >
    ...
    <button
      class="w-full flex items-center justify-between py-0 text-left group"
    ...
    >
      ...
      <div class="flex items-center gap-1">
        ...
        {/if}
        <span
          class="text-surface-500 group-hover:text-surface-300 transition-colors"
        >
          {#if collapsed}
    ...
    {#if !collapsed}
      <div class="mt-0 pb-2">
        {#if loading}
          <div class="flex items-center justify-center py-2 text-surface-400">
            <Loader2 class="h-5 w-5 animate-spin mr-2" />
            <span class="text-sm">Generating suggestions...</span>
          </div>
        {:else if suggestions.length === 0}
          <div class="text-center py-1 text-surface-500 text-sm">
        ...
        {:else}
          <div class="space-y-1.5 pt-0.5">
            {#each suggestions as suggestion}
              <button
                class="w-full text-left card p-2.5 hover:bg-surface-700/50 transition-colors group"
                onclick={() => {
              ...
              >
                <div class="flex items-start gap-3">
                  <ArrowRight
                    class="h-4 w-4 mt-0.5 text-surface-500 group-hover:text-primary-400 transition-colors shrink-0"
                  />
                  <div class="flex-1 min-w-0">
                    <p
                      class="text-sm text-surface-200 group-hover:text-surface-100"
                    >
                      {suggestion.text}
                    </p>
                    <span
                      class="text-xs {typeColors[
                        suggestion.type
                      ]} mt-1 inline-block"
                    >
